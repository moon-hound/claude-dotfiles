...
Options:
  --config/-c PATH   Path to HA config directory (default: config)
  --domain/-d DOM    Show only entities from specific domain
  --area/-a AREA     Show only entities from specific area
  --search/-s TERM   Search entities by name/id/device_class
  --full/-f          Show full detailed output

Pre-condition: requires a local config/ dir with .storage/ pulled from HA:
  rsync -avz root@homeassistant.local:/config/.storage/ config/.storage/
"""
...
def categorize_entities(entities: List[Dict], area_names: Dict[str, str]) -> Dict:
    ...
    key_domains = {
        ...
        "person", "device_tracker", "binary_sensor", "sensor", "media_player",
    ...
    for entity in entities:
        ...
        elif domain == "sensor" and device_class in ["temperature", "humidity", "motion", "door", "window"]:
            automation_relevant["sensor"].append(entity_info)
        elif domain == "binary_sensor" and device_class in ["motion", "door", "window", "occupancy"]:
...
def print_summary(categorized: Dict):
    ...
    total_entities = sum(len(e) for e in categorized["by_domain"].values())
...
def print_detailed_by_domain(categorized: Dict, domain_filter: Optional[str] = None):
    ...
    for domain in domains_to_show:
        ...
        for entity in sorted(entities, key=lambda x: x["entity_id"]):
            area_str = f" | {entity['area']}" if entity["area"] != "No Area" else ""
            unit_str = f" [{entity['unit']}]" if entity.get("unit") else ""
            device_class_str = f" ({entity['device_class']})" if entity.get("device_class") else ""
...
def print_by_area(categorized: Dict, area_filter: Optional[str] = None):
    ...
    for area in areas_to_show:
        ...
        for domain in sorted(by_domain_in_area.keys()):
            ...
            entity_ids = ", ".join(
                e["entity_id"] for e in sorted(domain_entities, key=lambda x: x["entity_id"])
            )
    ...
    for domain_entities in categorized["by_domain"].values():
        for entity in domain_entities:
            if (
                ...
                or (entity.get("device_class") and query_lower in entity["device_class"].lower())
            ):
...
def main():
    parser = argparse.ArgumentParser(description="Explore Home Assistant Entity Registry")
    parser.add_argument("--config", "-c", default="config", help="Path to HA config directory")
    parser.add_argument("--domain", "-d", help="Show only entities from specific domain")
    parser.add_argument("--area", "-a", help="Show only entities from specific area")
    parser.add_argument("--search", "-s", help="Search entities by name/id/device_class")
    parser.add_argument("--full", "-f", action="store_true", help="Show full detailed output")
